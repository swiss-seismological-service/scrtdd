...
unsigned int lsmrBase::GetStoppingReason() const { return this->istop; }
...
std::string lsmrBase::GetStoppingReasonMessage() const
{
  ...
  switch (this->istop)
  {
  case 0: msg = "The exact solution is  x = 0"; break;
  case 1: msg = "Ax - b is small enough, given atol, btol"; break;
  case 2: msg = "The least-squares solution is good enough, given atol"; break;
  case 3: msg = "The estimate of cond(Abar) has exceeded conlim"; break;
  case 4: msg = "Ax - b is small enough for this machine"; break;
  case 5: msg = "The LS solution is good enough for this machine"; break;
  case 6: msg = "Cond(Abar) seems to be too large for this machine"; break;
  case 7: msg = "The iteration limit has been reached"; break;
  default: msg = "Error. Unknown stopping reason"; break;
  }
  return msg;
}
...
double lsmrBase::GetFrobeniusNormEstimateOfAbar() const { return this->normA; }
...
double lsmrBase::GetFinalEstimateOfNormRbar() const { return this->normr; }
...
double lsmrBase::GetFinalEstimateOfNormOfX() const { return this->normx; }

void lsmrBase::SetLocalSize(unsigned int n) { this->localSize = n; }

void lsmrBase::SetEpsilon(double value) { this->eps = value; }

void lsmrBase::SetDamp(double value) { this->damp = value; }

void lsmrBase::SetToleranceA(double value) { this->atol = value; }

void lsmrBase::SetToleranceB(double value) { this->btol = value; }
...
void lsmrBase::SetOutputStream(std::ostream &os) { this->nout = &os; }
 ...
 *
 */
void lsmrBase::Solve(unsigned int m, unsigned int n, const double *b, double *x)
{
  const double zero = 0.0;
  ...
  if (this->nout)
  {
    ...
    (*this->nout) << " atol   = " << this->atol << ", conlim = " << this->conlim
                  << std::endl;
    (*this->nout) << " btol   = " << this->btol << ", itnlim = " << this->itnlim
                  << std::endl;
    (*this->nout)
  ...
  if (this->nout)
  {
    ...
    (*this->nout) << this->itn << ", " << x[0] << ", " << this->normr << ", "
                  << this->normA << ", " << test1 << ", " << test2 << std::endl;
  }
  ...
  do
  {
    ...
    if (beta > zero)
    {
      ...
      if (localOrtho)
      {
        ...
        {
          double d =
              std::inner_product(v, v + n, localV + n * localOrthoCount, 0.0);
    ...
    if (this->nout)
    {
      ...
      { // Print a line for this iteration
        ...
        pcount = pcount + 1;
        (*this->nout) << this->itn << ", " << x[0] << ", " << this->normr
                      << ", " << this->normAr << ", " << test1 << ", " << test2
                      << ", " << this->normA << ", " << this->condA
                      << std::endl;
      }
    }
...
void lsmrBase::TerminationPrintOut()
{
  ...
  if (this->nout)
  {
    (*this->nout) << " Exit  LSMR.       istop  = " << this->istop
                  ...
                  << " Exit  LSMR.       normA  = " << this->normA
                  << "     ,condA  = " << this->condA << std::endl
                  << " Exit  LSMR.       normb  = " << this->normb
                  << "     ,normx  = " << this->normx << std::endl
                  << " Exit  LSMR.       normr  = " << this->normr
                  << "     ,normAr = " << this->normAr << std::endl
                  << " Exit  LSMR.       " << this->GetStoppingReasonMessage()
                  << std::endl;
  }
}
