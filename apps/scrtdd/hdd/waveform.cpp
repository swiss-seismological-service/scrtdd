...
string
waveformDebugPath(const string& wfDebugDir, const Catalog::Event& ev, 
                  const Catalog::Phase& ph, const std::string& ext)
{
...
std::string
getBandAndInstrumentCodes(const std::string& channelCode)
{
...
std::string
getOrientationCode(const std::string& channelCode)
{
...
std::string
waveformId(const Core::TimeWindow& tw,
...
std::string
waveformId(const HDD::Catalog::Phase& ph, const Core::TimeWindow& tw)
{
    return waveformId(tw, ph.networkCode, ph.stationCode, ph.locationCode, ph.channelCode);
}


GenericRecordPtr
readWaveformFromRecordStream(
    const string& recordStreamURL, const Core::TimeWindow& tw,
    ...
    const string& locationCode, const string& channelCode)
{
  ...
  if (!trim(*trace, tw)) {
    string msg = stringify(
        "Incomplete trace, not enough data for requested"
...
bool
merge(GenericRecord& trace, const RecordSequence& seq)
{
  if (seq.empty()) {
    return false;
  }
...
bool
trim(GenericRecord& trace, const Core::TimeWindow& tw)
{
...
void
filter(GenericRecord& trace, bool demeaning,
       const std::string& filterStr, double resampleFreq)
{
...
void
resample(GenericRecord& trace, double sf, bool average)
{
  if (sf <= 0) return;
...
void
writeTrace(GenericRecordCPtr trace, const std::string& file)
{
  if (!trace) return;
...
GenericRecordPtr
readTrace(const std::string& file)
{
 ...
 */
bool
xcorr(const GenericRecordCPtr& tr1, const GenericRecordCPtr& tr2, double maxDelay,
      bool qualityCheck, double& delayOut, double& coeffOut)
{
    coeffOut = std::nan("");
...
double 
computeSnr(const GenericRecordCPtr& tr, const Core::Time& pickTime,
           ...
           double signalOffsetStart, double signalOffsetEnd)
{
    ...
    {
        SEISCOMP_ERROR("Cannot compute SNR: noise/signal windows exceed waveform boundaries");
        return -1;
    }
...
GenericRecordPtr
Loader::readAndProjectWaveform(const Core::TimeWindow& tw,
                               ...
                               const Catalog::Event& ev)
{
    ...
    if ( loc )
    {
        ...
        allComponents = getThreeComponents(tc, loc, channelCodeRoot.c_str(), tw.startTime());
    ...
    {
        string msg = stringify("Unable to fetch orientation information (%s)", wfDesc.c_str());
        throw runtime_error(msg);
    }
    ...
    // Load the components
    //
    auto loadWaveform = [this](const string& networkCode,const string& stationCode, 
                               const string& locationCode, const string& channelCode,
                               const Core::TimeWindow& tw)
    {
        ...
        {
            trace = getFromCache(tw, networkCode, stationCode, locationCode, channelCode);
            if ( trace ) _counters_wf_cached++;
        }
        if ( !trace )
        {
            trace = readWaveformFromRecordStream(
                _recordStreamURL, tw, networkCode, stationCode, locationCode, channelCode
            );
            _counters_wf_downloaded++;
        }
        if ( trace && _doCaching && !_cacheProcessed )
        {
            storeInCache(tw, networkCode, stationCode, locationCode, channelCode, trace);
        }
        return trace;
    };
    GenericRecordCPtr tr1 = loadWaveform(ph.networkCode, ph.stationCode, ph.locationCode,
                                        tc.comps[ThreeComponents::Vertical]->code(), tw);
    GenericRecordCPtr tr2 = loadWaveform(ph.networkCode, ph.stationCode, ph.locationCode,
                                        tc.comps[ThreeComponents::FirstHorizontal]->code(), tw);
    GenericRecordCPtr tr3 = loadWaveform(ph.networkCode, ph.stationCode, ph.locationCode,
...
GenericRecordCPtr
Loader::get(const Core::TimeWindow& tw,
            const Catalog::Phase& ph,
            const Catalog::Event& ev,
            bool demeaning,
            ...
            double resampleFreq)
{
    ...
    else
    {
        ...
        {
            storeInCache(tw, ph.networkCode, ph.stationCode,
                         ph.locationCode, ph.channelCode, trace);
        }
...
bool
DiskCachedLoader::isCached(const Core::TimeWindow& tw,
                           const Catalog::Phase& ph,
                           const Catalog::Event& ev)
{
    const std::string cacheFile = waveformPath(
        _cacheDir, tw, ph.networkCode, ph.stationCode, ph.locationCode, ph.channelCode);
...
void
DiskCachedLoader::storeInCache(const Core::TimeWindow& tw,
                               const std::string& networkCode,
                               const std::string& stationCode,
                               const std::string& locationCode,
                               const std::string& channelCode,
...
std::string
DiskCachedLoader::waveformPath(const std::string& cacheDir,
                               const Core::TimeWindow& tw,
                               const std::string& networkCode,
                               const std::string& stationCode,
                               const std::string& locationCode,
                               const std::string& channelCode) const
{
    std::string cacheFile =
        waveformId(tw, networkCode, stationCode, locationCode, channelCode) + ".mseed";
    return (boost::filesystem::path(cacheDir) / cacheFile).string();
}


bool
MemCachedLoader::isCached(const Core::TimeWindow& tw,
                          const Catalog::Phase& ph,
                          const Catalog::Event& ev)
{
...
GenericRecordCPtr
MemCachedLoader::getFromCache(const Core::TimeWindow& tw,
                              const std::string& networkCode,
                              const std::string& stationCode,
                              const std::string& locationCode,
                              const std::string& channelCode)
{
    const string wfId = waveformId(tw, networkCode, stationCode, locationCode, channelCode);
    const auto it = _waveforms.find(wfId);
    return it != _waveforms.end() ? it->second : nullptr;
}

void
MemCachedLoader::storeInCache(const Core::TimeWindow& tw,
                              const std::string& networkCode,
                              const std::string& stationCode,
                              const std::string& locationCode,
                              const std::string& channelCode,
...
GenericRecordCPtr
ExtraLenLoader::get(const Core::TimeWindow& tw,
                      const Catalog::Phase& ph,
                      const Catalog::Event& ev,
                      bool demeaning,
                      const std::string& filterStr,
                      double resampleFreq)
{
    ...
    GenericRecordCPtr trace = Loader::get(
            twToLoad, ph, ev, demeaning, filterStr, resampleFreq
    );
    if ( trace && twToLoad != tw )
    {
        ...
        {
            SEISCOMP_DEBUG("Incomplete trace, not enough data (%s)", string(ph).c_str());
            return nullptr;
        }
        trace = nonConstTrace;
    }
...
bool
SnrFilteredLoader::goodSnr(const GenericRecordCPtr& trace, const Core::Time& pickTime) const
{
...
GenericRecordCPtr
SnrFilteredLoader::get(const Core::TimeWindow& tw,
                      const Catalog::Phase& ph,
                      const Catalog::Event& ev,
                      bool demeaning, 
                      const std::string& filterStr,
                      double resampleFreq)
{
    ...
    if ( _snrGoodWfs.count(wfId) == 0 )
    {
        if ( ! goodSnr(trace, ph.time) )
        {
            ...
            {
                SEISCOMP_DEBUG("Trace has too low SNR, discard it (%s)", string(ph).c_str());
                writeTrace(trace, waveformDebugPath(_wfDebugDir, ev, ph, "snr-rejected") );
            }
    ...
    if ( twToLoad != tw )
    {
        ...
        if ( ! trim(*nonConstTrace, tw) )
        {
            SEISCOMP_DEBUG("Error when checking SNR, cannot trim data (%s)", string(ph).c_str());
            return nullptr;
        }
        trace = nonConstTrace;
    }
