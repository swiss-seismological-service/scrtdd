...
GenericRecordPtr readWaveformFromRecordStream(
      const std::string& recordStreamURL, const Core::TimeWindow& tw,
...
double computeSnr(const GenericRecordCPtr& tr, const Core::Time& pickTime,
...
std::string waveformId(const HDD::Catalog::Phase& ph, const Core::TimeWindow& tw);
...
class Loader : public Core::BaseObject
{
 ...
 public:
  ...
  void setDebugDirectory(const std::string& directory="") { _wfDebugDir = directory; }
 ...
 protected:
  ...
  Loader(LoaderPtr auxLdr, bool doCaching, bool cacheProcessed)
      : _auxLdr(auxLdr), _doCaching(doCaching),
        _cacheProcessed(cacheProcessed)
  {
...
class DiskCachedLoader : public Loader
{
 public:
  ...
  DiskCachedLoader(const std::string& recordStream, bool cacheProcessed,
                   const std::string& cacheDir)
      : Loader(recordStream, true, cacheProcessed), _cacheDir(cacheDir) {}

  DiskCachedLoader(LoaderPtr auxLdr, bool cacheProcessed,
                   const std::string& cacheDir)
...
class SnrFilteredLoader : public Loader
{
 ...
 public:
  ...
  SnrFilteredLoader(const std::string& recordStream, double minSnr,
                    double noiseStart, double noiseEnd,
                    double signalStart, double signalEnd)
      : Loader(recordStream), 
        _snr{minSnr, noiseStart, noiseEnd, signalStart, signalEnd} { }

  SnrFilteredLoader(LoaderPtr auxLdr, double minSnr,
                    double noiseStart, double noiseEnd, 
                    double signalStart, double signalEnd)
      : Loader(auxLdr),
        _snr{minSnr, noiseStart, noiseEnd, signalStart, signalEnd} { }
  ...
  bool goodSnr(const GenericRecordCPtr& trace, const Core::Time& pickTime) const;
