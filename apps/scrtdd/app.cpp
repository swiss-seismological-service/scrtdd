...
template <typename T>
void readConfig(const Seiscomp::Client::Application *,
                T *storage,
                const char *name)
{}
...
#define IMPL_READ_CONFIG(T, method) \
  template <> \
  void readConfig<T>(const Seiscomp::Client::Application *app, T *storage, \
                     const char *name) \
  { \
    try \
    { \
...
void readCLI<bool>(const Seiscomp::Client::CommandLine *cli,
                   ...
                   const char *name)
{
  if (cli->hasOption(name)) *storage = true;
}
...
template <typename T> struct OptionImpl : Application::Option
{
  OptionImpl(T *var,
      ...
      : Application::Option(
            cfgname, cligroup, cliparam, clidesc, clidefault, cliswitch),
        storage(var)
  {}
  ...
  void bind(Seiscomp::Client::CommandLine *cli)
  {
    ...
    if (cliSwitch)
      cli->addOption(cliGroup ? cliGroup : "Generic", cliParam, cliDesc);
    else
      cli->addOption(cliGroup ? cliGroup : "Generic", cliParam, cliDesc,
                     storage, cliDefault);
  }
...
{
  OptionVecImpl(std::vector<T> *var,
                const char *cfgname,
                const char *cligroup = nullptr,
                const char *cliparam = nullptr,
                const char *clidesc  = nullptr,
...
template <typename T>
Application::OptionPtr bind(T *var,
                            const char *cfgname,
                            const char *cligroup = nullptr,
                            const char *cliparam = nullptr,
                            const char *clidesc  = nullptr,
...
template <typename T>
Application::OptionPtr bind(std::vector<T> *var,
                            const char *cfgname,
                            const char *cligroup = nullptr,
                            const char *cliparam = nullptr,
                            const char *clidesc  = nullptr,
...
void Application::addOption(OptionPtr opt) { _options.push_back(opt); }

const Application::Options &Application::options() const { return _options; }
...
#define IMPL_ADD_OPTION(TYPE) \
  void Application::addOption(TYPE *var, const char *cfgname, \
                              ...
                              bool cliswitch) \
  { \
    addOption(bind(var, cfgname, cligroup, cliparam, clidesc, clidefault, \
                   cliswitch)); \
  }
