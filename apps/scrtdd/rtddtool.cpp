...
bool RTDD::run()
{
  ...
  if (!_config.dumpClusters.empty())
  {
    HDD::Catalog catalog = getCatalog(_config.dumpClusters);
  ...
  if (!_config.dumpWaveforms.empty())
  {
    HDD::Catalog catalog = getCatalog(_config.dumpWaveforms);
    ProfilePtr profile   = getProfile(_config.forceProfile);
    if (catalog.empty() || !profile) return false;
    loadProfile(profile, catalog);
  ...
  if (!_config.relocateCatalog.empty())
  {
    ...
    HDD::Catalog catalog = getCatalog(_config.relocateCatalog, &idmap);
    ProfilePtr profile   = getProfile(_config.forceProfile);
    if (catalog.empty() || !profile) return false;
    ...
    if (commandline().hasOption("xmlout"))
    {
      ...
      {
        HDD::Catalog ev = relocatedCat.extractEvent(kv.second.id, true);
                         ...
                         const ProfilePtr &profile)
{
  ...
  // Skip automatic or manual origins if configured so
  if (!_config.forceProcessing)
  {
    ...
    if (isManualOrigin && !_config.allowManualOrigin)
    {
      SEISCOMP_DEBUG("Ignoring manual origin %s", origin->publicID().c_str());
      return false;
    }
    if (!isManualOrigin && !_config.allowAutomaticOrigin)
    {
  ...
  if (!_config.acceptedOriginAuthors.empty() && !_config.forceProcessing)
  {
    ...
    for (const string &allowedAuthor : _config.acceptedOriginAuthors)
    {
      try
      {
        if (origin->creationInfo().author() == allowedAuthor)
        {
          authorAllowed = true;
          break;
        }
    ...
    if (!authorAllowed)
    {
      SEISCOMP_DEBUG("Ingoring origin %s whose author is not accepted",
                     origin->publicID().c_str());
      return false;
    }
  }
                         ...
                         const HDD::Catalog alternativeCatalog)
{
  ...
  try
  {
    // load the catalog
    HDD::Catalog ddbgc;
    if (!alternativeCatalog.empty()) // force this catalog
    {
