                               ...
                               const CatalogPtr& refEvCatalog)
{
    ...
    //
    map<MissingStationPhase,const Catalog::Phase*> missingPhases = getMissingPhases(searchCatalog, refEv, refEvCatalog, fixAutoPhase);
    ...
    for ( const auto& kv : missingPhases )
    {
        ...
        //
        vector<HypoDD::PhasePeer> xcorrPeers = findPhasePeers(station, phaseType, searchCatalog, eventsInRange);

        //
        // If we have an existing phase first try to xcorr the existing pick time and if the result
        // is good enough, don't try to detect the pick against theoretical pick with cross correlation
        //
        if ( existingPhase )
        {
            unsigned goodCC = 0;
...
vector<unsigned>
HypoDD::getEventsInRange(const Catalog::Event& refEv, const CatalogCPtr& searchCatalog, double maxHorizDist) const
{
    //
    // Select events in maxIEdist range
    //
    ...
    HddEllipsoid ellipsoid(0, maxHorizDist, refEv.latitude, refEv.longitude, refEv.depth);
...
HypoDD::findPhasePeers(const Catalog::Station& station, const std::string& phaseType,
                       const CatalogCPtr& searchCatalog, 
                       const vector<unsigned>& eventsInRange) const
{
    //
...
bool
HypoDD::detectPhase(bool useXCorr, unsigned numCC,
                    const Catalog::Station& station,
                    const string& phaseType,
                    const Catalog::Event& refEv,
                    const CatalogCPtr& searchCatalog,
                    const vector<HypoDD::PhasePeer>& xcorrPeers,
                    ...
                    Catalog::Phase& refEvNewPhase /*output */ )
{
    ...
    {
        GenericRecordPtr refTr = getWaveform(xcorrTw, refEv, refEvNewPhase, _wfCacheTmp, false, false);
...
{
    CatalogPtr catalog = new Catalog(*_ddbgc);

    if ( _cfg.artificialPhases.enable )
    {
        addMissingPhases(_cfg.artificialPhases.useXCorr,  _cfg.artificialPhases.fixAutoPhase,
                         _cfg.artificialPhases.maxIEdist, _cfg.artificialPhases.numCC,
                         catalog, catalog);
    }
    ...
    for (const auto& kv : catalog->getStations() )
    {
        ...
        SEISCOMP_INFO("Starting cross-correlation on station %s.%s",  station.networkCode.c_str(), station.stationCode.c_str() );
        unsigned amount = _counters.xcorr_tot;

        //
        // Cross correlate each event phase on this station against each other event
        //
        for (auto it1 = catalog->getEvents().begin(); it1 != catalog->getEvents().end(); it1++ )
        {
            const Catalog::Event& ev1 = it1->second;

            // loop through event 1 phases and find current station
            auto eqlrng = catalog->getPhases().equal_range(ev1.id);
            for ( auto phit1 = eqlrng.first; phit1 != eqlrng.second; ++phit1 )
            {
                const Catalog::Phase& ph1 = phit1->second;

                if (ph1.stationId == station.id )
                {
                    // loop through the other events and find phases for the same station
                    for (auto it2 = std::next(it1); it2 != catalog->getEvents().end(); it2++ )
                    {
                        const Catalog::Event& ev2 = it2->second;

                        // loop through event 2 phases and find current station and same event 1 phase type
                        auto eqlrng2 = catalog->getPhases().equal_range(ev2.id);
                        for (auto phit2 = eqlrng2.first; phit2 != eqlrng2.second; ++phit2)
                        {
                            const Catalog::Phase& ph2 = phit2->second;

                            if (ph1.stationId     == ph2.stationId  &&
                                ph1.procInfo.type == ph2.procInfo.type)
                            {
                                double coeff, lag, weight;
                                xcorrPhases(ev1, ph1, true, _wfCache, _useCatalogDiskCache,
                                            ev2, ph2, true, _wfCache, _useCatalogDiskCache,
                                            coeff, lag, weight);
                                break;
                            }
                        }
        ...
        if ( amount == _counters.xcorr_tot )
        {
            SEISCOMP_INFO("Nothing to do for this station");
            continue;
        }
        printCounters();
    }
