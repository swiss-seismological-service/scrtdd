...
bool RTDDLocator::init(const Config::Config &config)
{
  ...
  try
  {
    _profileNames = config.getStrings("rtddloc.activeProfiles");
  }
  catch (...)
  {}
...
bool RTDDLocator::setParameter(const string &name, const string &value)
{
...
Origin *RTDDLocator::relocate(const Origin *origin)
{
  ...
  if (!requestAccepted)
  {
    if (lastError.empty())
      throw LocatorException(
          "No reply from scrtdd. Has scrtdd module been started?");
    else
